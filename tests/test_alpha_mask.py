...
def test_mask_exact1(test_fixture):
    ...
    assert np.array_equal(
        np.invert(np.all(np.rollaxis(img, 0, 3) == 255, axis=2)).astype(img.dtype)
...
def test_mask_exact2(test_fixture2):
    ...
    assert mask_exact(img, ndv).dtype == img.dtype
    assert np.array_equal(
        np.invert(np.all(np.rollaxis(img, 0, 3) == ndv, axis=2)).astype(img.dtype)
        * np.iinfo(img.dtype).max,
        mask_exact(img, ndv),
    )


arr1 = arrays(np.uint8, (3, 8, 10), elements=st.integers(min_value=1, max_value=1))
...
def test_mask_exact3(arr):
    ...
    assert np.array_equal(
        np.invert(np.all(np.rollaxis(arr, 0, 3) == ndv, axis=2)).astype(arr.dtype)
...
def test_mask_exact4():
    ...
    assert np.array_equal(
        np.any(np.rollaxis(arr2, 0, 3) != ndv, axis=2),
